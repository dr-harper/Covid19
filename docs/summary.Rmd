...
p2 <- ggplot(df_sum_continent, aes(x = date, y = value, fill = continent)) + 
  ...
  labs(title = "Number of daily recorded deaths",
       ...
       x = "Date",
       fill = "Continent") +
  scale_y_log10(expand = c(0,0), limits = c(1, max(df_sum_continent$value * 10)), breaks = c(1, 10, 100, 1000, 10000, 100000, 1000000)) + 
  theme(panel.grid.major.x = element_blank(),
        legend.background = element_blank(),
        legend.position = "none") +
  scale_x_datetime(limits = c(min(df_sum_continent$date), max(df_sum_continent$date) + lubridate::ddays(7)))
p2
```
