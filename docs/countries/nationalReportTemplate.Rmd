...
if(nrow(df_country_bar) == 0) df_country_bar <- df_country
...
p2 <- ggplot(df_country_bar) + 
  ...
  labs(title = "Daily number of recorded cases and deaths",
       ...
       Total number of deaths: {deaths}", deaths = metrics$deaths, cases = metrics$cases)) +
...
baseplot <- 
  ...
  geom_point(data = df2_selectedcountry_plot2, colour = "#3a6589", size = 2) +
  geom_label(data = df2_selectedcountry_plot2, aes(label = area), colour = "#3a6589", vjust = 2) +
  labs(x = "Daily increase in number of deaths",
       y = "% Change in deaths",
       fill = "Country",
       title = glue::glue("COVID-19 related deaths: {country} vs. other countries", country = params$country),
       caption = caption) +
